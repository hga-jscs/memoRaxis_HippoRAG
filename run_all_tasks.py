...
用于串联 HippoRAG 子项目在 MemoryAgentBench 上的四个任务：
...
def preprocess_preview_samples(python_exe: str) -> None:
    cmd = [
        python_exe,
        "scripts/HippoRAG_MAB/data/convert_all_data.py",
    ]
    run_cmd(cmd)
...
def ingest_acc(
    ...
    cmd = [
        python_exe,
        "scripts/HippoRAG_MAB/ingest/ingest_accurate_retrieval.py",
        "--instance_idx",
        instance_idx,
        "--chunk_size",
        str(chunk_size),
...
def infer_acc(
    ...
    cmd = [
        python_exe,
        "scripts/HippoRAG_MAB/infer/infer_accurate_retrieval.py",
        "--instance_idx",
        instance_idx,
        "--adaptor",
        *adaptors,
...
def eval_acc(
    ...
    for idx in parse_indices(instance_idx):
        ...
        cmd = [
            python_exe,
            "scripts/HippoRAG_MAB/evaluate/evaluate_mechanical.py",
            "--results",
            str(result_file),
            "--instance",
            str(gt_file),
...
def ingest_conflict(
    ...
    cmd = [
        python_exe,
        "scripts/HippoRAG_MAB/ingest/ingest_conflict_resolution.py",
        "--instance_idx",
        instance_idx,
        "--min_chars",
        str(min_chars),
...
def infer_conflict(
    ...
    cmd = [
        python_exe,
        "scripts/HippoRAG_MAB/infer/infer_conflict_resolution.py",
        "--instance_idx",
        instance_idx,
        "--adaptor",
        *adaptors,
...
def eval_conflict(python_exe: str) -> None:
    cmd = [
        python_exe,
        "scripts/HippoRAG_MAB/evaluate/evaluate_conflict_official.py",
    ]
    run_cmd(cmd)
...
def ingest_long(
    ...
    cmd = [
        python_exe,
        "scripts/HippoRAG_MAB/ingest/ingest_long_range.py",
        "--instance_idx",
        instance_idx,
        "--chunk_size",
        str(chunk_size),
...
def infer_long(
    ...
    cmd = [
        python_exe,
        "scripts/HippoRAG_MAB/infer/infer_long_range.py",
        "--instance_idx",
        instance_idx,
        "--adaptor",
        *adaptors,
...
def eval_long(
    ...
    for idx in parse_indices(instance_idx):
        ...
        cmd = [
            python_exe,
            "scripts/HippoRAG_MAB/evaluate/evaluate_long_range_A.py",
            "--results",
            str(result_file),
            "--instance_folder",
...
def ingest_ttl(
    ...
    cmd = [
        python_exe,
        "scripts/HippoRAG_MAB/ingest/ingest_test_time.py",
        "--instance_idx",
        instance_idx,
        "--save_root",
        save_root,
...
def infer_ttl(
    ...
    cmd = [
        python_exe,
        "scripts/HippoRAG_MAB/infer/infer_test_time.py",
        "--instance_idx",
        instance_idx,
        "--adaptor",
        *adaptors,
...
def eval_ttl(
    ...
    cmd = [
        python_exe,
        "scripts/HippoRAG_MAB/evaluate/evaluate_ttl_mechanical.py",
        "--results_pattern",
        pattern,
    ]
    run_cmd(cmd)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run all 4 HippoRAG tasks end-to-end")
    ...
    parser.add_argument("--index_root", type=str, default="out/hipporag_indices")
    ...
    parser.add_argument("--force_rebuild", action="store_true", help="ingest 前删除已有 index_dir 并重建")
    ...
    parser.add_argument("--acc_max_chunks", type=int, default=-1)
    ...
    parser.add_argument("--conflict_max_chunks", type=int, default=-1)
    ...
    parser.add_argument("--long_max_chunks", type=int, default=-1)
